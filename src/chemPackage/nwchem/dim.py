    ...
    if start:
        ...
        for st in start:
            ...
            if '---------------' in f[st+1]:
                ...
                # Store the different direction sequentially for each
                dir = { 'X': 0, 'Y': 1, 'Z': 2 }[f[st+1].split()[0].upper()]
                try:
...
#   TODO: Come back and see about fixing collecting the int. energy
#    ###################################
...
#
    ...
    if 'DIM System Energy' in indices:
        s = indices['DIM System Energy']
        ix = next(i for i, x in enumerate(f[s:], s) if '  Total' in x)
