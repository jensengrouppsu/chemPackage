...
def collect_optimized_geometry(self, f, indices):
    ...
    if 'OPTIMIZED GEOMETRY' in indices:
        ...
        self.initial_geometry = array(
                                [x.split()[5:8] for x in f[s:e]], dtype=float)

        # Final geometry
        s = indices['OPTIMIZED GEOMETRY'][-1]
        e = s + self.natoms
        # Elements 5, 6 and 7 are the X, Y, and Z coordinates
        self.coordinates = array([x.split()[5:8] for x in f[s:e]],dtype=float)
    else:
